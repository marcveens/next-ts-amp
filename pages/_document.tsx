...
type HybridDocumentProps = {
  amp: boolean;
};

export default class HybridDocument extends React.Component<HybridDocumentProps> {
  static getInitialProps ({ renderPage }: GetInitialProps) {
  ...
  render () {
    if (this.props.amp) {
      // @ts-ignore amp is no valid attribute, yet
      return <html amp="">
        <AmpHead />
        <body>
          <Main />
...
export class AmpHead extends Component {
  ...
  render () {
    ...
    return <head {...this.props}>
      ...
      {(head || []).map((h: JSX.Element, i: number) => React.cloneElement(h, { key: i }))}
      {styles || null}
      {this.props.children}
    </head>
  }
